local npcs = {"npc_combine_s"}

function basedefence.SpawnGred()
	for _, point in pairs(ReadDataMap("basedefencegred")) do
	...
	for _, point in pairs(ReadDataMap("basedefencegred_ammo")) do
	...
	for _, point in pairs(ReadDataMap("gred_simfphys_brdm2")) do
	...
	timer.Create("BD_npcwave", 60, 0, function()
		...
		for _ = 1, #botsspawns - count do
			...
			bot:CallOnRemove("botdead", function()
				count = count - 1
				if timer.Exists("botsupdatemem" .. bot:EntIndex()) then timer.Remove("botsupdatemem" .. bot:EntIndex()) end
			end)
		end

		for _ = 1, #boxspawn - countBox do
...
function basedefence.RoundEndCheck()
	...
	if Alive == 0 then return EndRound() end
	if roundTimeStart + roundTime - CurTime() <= 0 then EndRound(1) end
end
...
local wepeno = {"weapon_mp7", "weapon_sar2"}
...
function basedefence.PlayerSpawn2(ply, teamID)
	...
	if math.random(1, 3) == 3 then ply:Give("weapon_hammer") end
...
function basedefence.SpectateNPC(ply, npc)
	if npc:GetClass() == "npc_combine_s" then
		...
		for _, ent in pairs(ents.GetAll()) do
			if ent:IsNPC() then ent:AddEntityRelationship(ply, D_LI, 99) end
		end
	end
end