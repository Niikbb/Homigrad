if ( string.find( string.lower( game.GetMap() ), "gm_site19" ) ) then
...
function CLASS.Looking(self,pos)
    ...
    --print(#players) -- Проверка видимости
    return players
end
...
if CLIENT then
    ...
    function CLASS:HUDPaint()
        ...
        local players = CLASS.Looking(self,self:EyePos())
        local count = #self.players or 0
        if count > 0 then
            draw.SimpleText("На тебя смотрят: " .. count .. " сущностей","HomigradFont",ScrW() / 2,25,white2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
        else
            draw.SimpleText("Тебя никто не видит","HomigradFont",ScrW() / 2,25,white2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
        end
    end
end
