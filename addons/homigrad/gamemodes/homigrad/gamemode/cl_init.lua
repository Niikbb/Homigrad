...
hook.Add("PostDrawOpaqueRenderables", "laser", function()
	...
	for i, ply in pairs(laserplayers) do
		...
		if IsValid(ply) and ply.Laser and not ply:GetNWInt("Otrub") and laserweps[IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() or ply.curweapon] then
			...
			if not tra.Hit then render.DrawSprite(tr.HitPos, Size, Size, Color(255, 0, 0)) end
...
local function ToggleMenu(toggle)
	if toggle then
		...
		if wep:GetClass() ~= "weapon_hands" then wepMenu:AddOption("Выкинуть", function() LocalPlayer():ConCommand("say *drop") end) end
		if wep:Clip1() > 0 then
		...
		plyMenu:AddOption("Меню Брони", function() LocalPlayer():ConCommand("jmod_ez_inv") end)
		plyMenu:AddOption("Меню Патрон", function() LocalPlayer():ConCommand("hg_ammomenu") end)
		local EZarmor = LocalPlayer().EZarmor
		if JMod.GetItemInSlot(EZarmor, "eyes") then plyMenu:AddOption("Активировать Маску/Забрало", function() LocalPlayer():ConCommand("jmod_ez_toggleeyes") end) end
	else
		if IsValid(wepMenu) then wepMenu:Remove() end
...
hook.Add("HUDShouldDraw", "HideHUD_ammo", function(name) if name == "CHudAmmo" then return false end end)
