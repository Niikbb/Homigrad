...
net.Receive("radio_use", function(len, ply)
	...
	if IsValid(radio) then
		...
		radpanel.Paint = function(self, w, h)
			if not IsValid(radio) or not LocalPlayer():Alive() then return radpanel:Remove() end
		...
		button2.Paint = function(self, w, h) draw.RoundedBox(0, 0, 0, w, h, Color(255, 0, 0)) end
		...
		button.Paint = function(self, w, h) draw.RoundedBox(0, 0, 0, w, h, Color(0, 255, 0)) end
	end
end)
...
net.Receive("play_snd", function(len)
	...
	if play then
		if not g_station[radio:EntIndex()] or not IsValid(g_station[radio:EntIndex()][1]) then
			if g_station[radio:EntIndex()] and not IsValid(g_station[radio:EntIndex()][1]) then link = link or g_station[radio:EntIndex()][2] end
			...
			sound.PlayURL(link, "3d", function(station, err, errName)
				if IsValid(station) then
					...
					print("[fun/radio] Error!", err or "", errName or "")
				end
			end)
		...
		if g_station[radio:EntIndex()] and IsValid(g_station[radio:EntIndex()][1]) then g_station[radio:EntIndex()][1]:Stop() end
	end
...
hook.Add("Think", "station_think", function()
	if stationThink < CurTime() then
		...
		for i, _ in pairs(g_station) do
			if IsValid(g_station[i][1]) then g_station[i][1]:SetPos(IsValid(g_station[i][3]) and g_station[i][3]:GetPos() or Vector(0, 0, 0)) end
		end
	end
end)