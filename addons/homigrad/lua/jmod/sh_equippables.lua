		...
		for k, ply in player.Iterator() do
			if ply:Alive() and ply.EZequippables then
				for slot, info in pairs(ply.EZequippables) do
					...
					if TimeLeft <= 0 then
						if SERVER then
							JMod.SetEquippable(ply, slot, nil)
						end
					else
						if EquippableSpecs.thinkFunc then
							EquippableSpecs.thinkFunc(ply, info.col, TimeLeft)
						end
					end
				end
			end
...
if CLIENT then
	hook.Add("PostPlayerDraw", "JModEquippablePlayerDraw", function(ply)
		if ply.EZequippables then
			if not ply.EZequippableModels then
				ply.EZequippableModels = {}
			end
			...
			for slot, info in pairs(ply.EZequippables) do
				...
				if EquippableSpecs.drawFunc then
					EquippableSpecs.drawFunc(ply, info.col, info.tim - CurTime())
				end
			end
		end
	end)
