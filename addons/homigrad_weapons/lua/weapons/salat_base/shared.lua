...
hook.Add("HUDPaint", "admin_hitpos", function()
	if hg_show_hitposmuzzle:GetBool() and LocalPlayer():IsAdmin() then
		...
		surface.DrawRect(hit.x - 2.5, hit.y - 2.5, 2.5, 2.5)
	end
end)
...
hook.Add("Think", "Mul lerp", function() mul = FrameTime() / TickInterval() end)
...
function SWEP:Step()
	...
	if (CLIENT and isLocal) or SERVER then
		if not ply:GetNWBool("Suiciding") and not ply:IsSprinting() then
			...
			if eyeangles[1] > numbr then hand[1] = hand[1] - (eyeangles[1] - numbr) end
			if eyeangles[1] < -numbr then hand[1] = hand[1] - (eyeangles[1] + numbr) end
		end
	end
	...
	if not ply:LookupBone("ValveBiped.Bip01_R_Forearm") then --;c
		return
	end
